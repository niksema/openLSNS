...
class cudaEventTimer{
    public:
	cudaEventTimer( void ) : IsStarted( false )
	{
...
#define PAR				/*actual parameters for cuda kernel*/
#define T_PAR				/*description of parameters for cuda kernel*/
#define T_KERNEL( f ) __global__ void( *f )( T_PAR ) 

__global__ void my_kernel( T_PAR )
{
}

int run_timemesure( T_KERNEL( pkernel ), dim3 blocks, dim3 threads, size_t mem = 0, cudaStream_t stream = cudaStream_t() )
{
	cudaEventTimer ctimer;
	cTimer.start();
	( *pkernel )<<<blocks, threads, mem, stream>>>( PAR );
	cTimer.stop();
	printf("Executing time in msec: %.2f\n", cTimer.elapsed() );
};
